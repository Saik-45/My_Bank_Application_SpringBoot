        ...
        if (customers.isEmpty()) {
            ...
            for (Customer customer : customers) {
                response.append("\n- Customer ID       : ").append(customer.getId()+"\n")
                        .append("- Full Name         : ").append(customer.getAccountHolderName()+"\n")
    ...
    public ResponseEntity<?> findCustomerByMobileNumber(@RequestParam String mobileNumber) {
        try {
            ...
            response.append(mobileNumber+" Mobile Number Account Details \n\n")
                    ...
                    .append("- Full Name         : ").append(customer.getAccountHolderName()).append("\n")
...
//        http://localhost:4545/bank/checkBalancer/98787965429?atmPin=2001
    public ResponseEntity<?> findBalance(@PathVariable Long accountNumber, @RequestParam String atmPin) {
        try {
            Double balance = customerService.checkBalance(accountNumber, atmPin);
            String accountHolderName;

            if (customerService.existsByAccountNumber(accountNumber)) {
                accountHolderName = customerService.getAccountHolderName(accountNumber);
            } else {
            ...
            if (balance != null) {
                StringBuilder response = new StringBuilder();
                response.append("\t\tChecked Balance With ATM PIN \n\n")
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Invalid Account Number or ATM pin");
            }
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Internal Server Error");
        }
    ...
    @GetMapping("/checkBalanceWith/{accountNumber}")
    //           http://localhost:4545/bank/checkBalance/98787965429
    public ResponseEntity<?> findBalanceWithAccountNumber(@PathVariable Long accountNumber) {
        try {
            Double balance = customerService.checkBalanceAccountNumber(accountNumber);
            String accountHolderName;
            ...
            if (balance != null) {
                StringBuilder response = new StringBuilder();
                response.append("\t\tChecked Balance With Account Number \n\n")
                    //    .append(accountNumber).append("\n\n")
                        .append("Account Holder Name : ").append(accountHolderName).append("\n")
                      //  .append("Account Number      : ").append(accountNumber).append("\n")
                        .append("       Balance      : ").append(balance);
