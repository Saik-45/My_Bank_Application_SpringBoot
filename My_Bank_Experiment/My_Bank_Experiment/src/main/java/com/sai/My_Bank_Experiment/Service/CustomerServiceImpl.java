...
public class CustomerServiceImpl implements CustomerService {
   ...
   @Autowired
    ...
    public void createAccount(Customer customer) {
        LOGGER.info("Creating a new customer: " + customer.getAccountHolderName());
    ...
    @Override
    public Double checkBalance(Long accountNumber, String atmPin) {
        try {
            Customer customer = customerRepository.findByAccountNumber(accountNumber);

            if (customer != null && atmPin.equals(customer.getAtmPin())) {
                return customer.getAmount();
            } else {
                return null;
            }
        } catch (Exception e) {
            throw new RuntimeException("Error checking balance for account number: " + accountNumber, e);
        }
    }
    ...
    public String getAccountHolderName(Long accountNumber) {
        Customer customer = customerRepository.findByAccountNumber(accountNumber);
        return (customer != null) ? customer.getAccountHolderName() : null;
    }

    @Override
    public Double checkBalanceAccountNumber(Long accountNumber) {
        Customer customer = customerRepository.findByAccountNumber(accountNumber);
        if (customer != null) {
            return customer.getAmount();
        } else {
